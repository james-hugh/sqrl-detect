 ...
 */
void init_system() {
    // Set umask to ensure all created files/dirs are owner-only
    umask(0077);
    srand(time(NULL));

    // Create log directory with owner-only permissions.
    // We call mkdir directly to avoid TOCTOU race conditions.
