...
void init_system() {
    srand(time(NULL));
    umask(0077); // Ensure files and directories are only readable by the owner
...
void view_logs() {
    ...
    printf("\n--- END OF SCROLLS ---\n");
    printf("\nPress ENTER to return to the Command Center...");
    // If the previous input left a newline, we might need to clear it.
    // However, in our main menu we use fgets which consumes the newline.
    // But if we used something else, we might need to be careful.
    getchar();
}
...
void engage_defenses() {
    ...
    while (1) {
        ...
        if (fort_integrity <= 0) {
            printf("\n%sFATAL ERROR: PILLOW FORT COMPROMISED.%s\n", RED, RESET);
            printf("The squirrels have breached the perimeter. The cows are fleeing.\n");
            log_event("CRITICAL FAILURE: PILLOW FORT INTEGRITY AT 0%. SYSTEM COMPROMISED.");
            printf("\nPress ENTER to retreat to your backup fort...");
            while (getchar() != '\n'); // Clear buffer
            getchar();
            break;
        }
...
int main() {
    ...
    while (1) {
        ...
        if (strcasestr(command, "ENGAGE DEFENSES") != NULL || strstr(command, "1") != NULL) {
            ...
            view_logs();
        } else if (strcasestr(command, "EXIT") != NULL || strstr(command, "3") != NULL) {
